    ...
    if not sender.bot:
        ...
        if not no_log_pms_sql.is_approved(chat.id) and chat.id != 777000:
            if LOG_CHATS_.RECENT_USER != chat.id:
                ...
                edited_message = f"الرسالة المعدلة: {event.message.text}" if event.message else "الرسالة المعدلة: N/A"
                
                await event.client.send_message(
                    Config.PM_LOGGER_GROUP_ID,
                    f"**🛂┊المسـتخـدم :** {_format.mentionuser(sender.first_name , sender.id)} **- قام بـ تعديل الرسالة** \n**🎟┊الايـدي :** `{chat.id}`\n\n{original_message}\n\n{edited_message}",
                )
